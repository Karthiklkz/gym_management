...
export default function TrainerNotificationsPage() {
  ...
  const { data: notificationsResponse, isLoading } = useQuery({
    queryKey: ["trainerNotifications", currentPage],
    queryFn: () => apiClient<any>(`/api/trainer/notifications?page=${currentPage}&limit=10`),
  });
  ...
  const markReadMutation = useMutation({
    mutationFn: (id: string) =>
      apiClient("/api/trainer/notifications", {
        method: "PUT",
        body: JSON.stringify({ id }),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["trainerNotifications"] });
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-400">View and manage system alerts and gym notifications.</p>
        </div>
        <button
      ...
      <div className="bg-[#090D16] border border-slate-800 rounded-xl p-6 shadow-md">
        {isLoading ? (
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className={`text-sm ${!item.isRead ? "font-bold text-white" : "font-semibold text-slate-300"}`}>
                        {item.title}
                      </h4>
                    </div>
                    <p className="text-slate-400 leading-relaxed pl-4">{item.message}</p>
                  </div>
                  <span className="text-[10px] text-slate-550 shrink-0 font-medium uppercase tracking-wide">
              ...
              <div className="flex justify-between items-center pt-6 border-t border-slate-850">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-350 hover:text-white rounded-md text-xs font-semibold disabled:opacity-40 disabled:cursor-not-allowed transition cursor-pointer"
                  >
                    Previous
                  </button>
                  <button
          ...
          <div className="text-center py-10">
            <p className="text-slate-450 font-medium text-sm">No notifications found</p>
